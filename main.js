...
for (let i = 0; i < buttonsBanks.length; i++) {
  buttonsBanks[i].addEventListener("click", () => {
    for (let i = 0; i < buttonsBanks.length; i++) {
      buttonsBabks[i].classList.remove('active');
    }
...
let monthlyPayment = (amountLoan + (((amountLoan / 100) *
  interestRate) / 12) * amountMonth) / amountMonth;
let monthlyPaymentRound = Math.round(monthlyPayment);
let recIncome = Math.round(monthlyPayement + ((monthlyPayment * 35) / 100))
